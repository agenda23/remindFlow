...
- ヘッダー通知メニュー：赤ドットは「当日これからのリマインダー件数＋未読履歴件数 > 0」で表示
...
- 高度な検索機能（日付範囲・AND/OR）は一覧画面に実装済み（状態保存/復元も実装）
...
- CSV項目に開始/終了時間とステータスを追加（`utils/storage.ts`）
 - 通知履歴（保存/表示/既読/全削除）、未読件数のバッジ反映を追加（`notifications.ts`/`storage.ts`/`App.jsx`）
 - ICSエクスポートを追加（`utils/storage.ts`/`SettingsModal.jsx`）
 - 作成時デフォルト（カテゴリ）を設定に追加し、フォーム/日付クリック作成へ反映（`SettingsModal.jsx`/`ScheduleForm.jsx`/`App.jsx`）
 - 一覧の高度検索（AND/OR・日付範囲）と状態保存/復元を追加（`ListView.jsx`）
...
3. 高度検索（AND/OR・日付範囲保存）（実装済）

### 中優先度（更新）
4. icsエクスポート/インポート対応（エクスポート実装済み）
5. レスポンシブ操作性の微調整
