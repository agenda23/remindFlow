...
##### 通知メニュー（ヘッダー）
- 赤いドットは当日これからのリマインダーがある場合のみ表示
- メニュー項目: 今すぐチェック / テスト通知 / 通知の有効化・無効化 / 権限リクエスト

##### 実装状況（通知）
- 通知許可リクエスト、リマインダーの定期チェック、当日分の通知スケジュール、通知音指定（音声ファイル配置が必要）、通知ON/OFF切り替え（設定モーダル）を実装済み。
  - 音声ファイル: `public/sounds/{chime,bell,notification}.mp3` を配置。
...
- ダッシュボードのクイックアクションから「カレンダー表示」「通知設定」への導線を実装。
...
- **インポート/エクスポート**
    ...
    - カレンダーファイル（.ics）対応（未実装）
        
    - データ復元機能（JSONバックアップ/インポート対応済み）

##### 実装状況（データ管理）
- 予定データはローカルストレージに簡易暗号化して保存。
- 設定モーダルからCSVエクスポート、JSONバックアップ出力/インポートを実装済み。
...
- JSONインポート/エクスポート: 実装済み（CSVエクスポート含む）
