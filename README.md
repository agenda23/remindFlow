...
- **オフライン対応**: 今後対応予定（PWA/Service Worker）
...
補足: 通知音は実行環境に応じて以下のURL候補から自動解決します。
1) `${BASE_URL}sounds/{name}.mp3` 2) `/sounds/{name}.mp3` 3) `sounds/{name}.mp3`
`BASE_URL` はビルド時は `/remindFlow/`、開発時は `/` です。

### 通知メニュー（ヘッダー）
- 通知アイコンの赤いドットは「当日これからのリマインダー数＋未読履歴件数 > 0」のときに表示
...
- 作成時デフォルト（カテゴリ）を設定できます（新規作成フォーム/カレンダー日付クリックに反映）。
...
### 一覧（ListView）の高度検索と状態保存
- フィルターを開くと、日付範囲（開始/終了）とAND/ORモードの切替が利用できます。
- 検索/フィルタ/ソート/日付範囲/AND-ORの状態はローカルに保存され、再訪時に復元されます。
